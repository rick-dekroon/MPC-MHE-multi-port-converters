close all; clear; clc;

% Converter parameters
p = MIMO_case_B_params();
nx = 3;
nu = 2;
ny = 2;

% Prediction horizon
N = 4;
N_MPC = N;

% Estimation horizon
N_MHE = 4;

% Initial state
x_0 = [0;
       0;
       0];

% Reference
iin_ref = 1;

r1 = [p.Vac;
      iin_ref];
r2 = [-p.Vac;
       iin_ref];

% Converter model
f1 = @(x, u) dynamics_f(x, u, r1, p);
g1 = @(x, u) dynamics_g(x, u, r1, p);
f2 = @(x, u) dynamics_f(x, u, r2, p);
g2 = @(x, u) dynamics_g(x, u, r2, p);

function xdot = dynamics_f(x, u, r, p)
    if r(1) >= 0
        [xdot, ~] = MIMO_case_B_dynamics_SM1(x, u, p);
    else
        [xdot, ~] = MIMO_case_B_dynamics_SM2(x, u, p);
    end
end

function y = dynamics_g(x, u, r, p)
    if r(1) >= 0
        [~, y] = MIMO_case_B_dynamics_SM1(x, u, p);
    else
        [~, y] = MIMO_case_B_dynamics_SM2(x, u, p);
    end
end

k1 = @(x, u) f1(x, u);
k2 = @(x, u) f1(x + p.Ts/2*k1(x,u), u);
k3 = @(x, u) f1(x + p.Ts/2*k2(x,u), u);
k4 = @(x, u) f1(x + p.Ts*k3(x,u), u);
fd1 = @(x, u) x + p.Ts/6*(k1(x,u) + 2*k2(x,u) + 2*k3(x,u) + k4(x,u));
gd1 = @(x, u) g1(x, u);

k1 = @(x, u) f2(x, u);
k2 = @(x, u) f2(x + p.Ts/2*k1(x,u), u);
k3 = @(x, u) f2(x + p.Ts/2*k2(x,u), u);
k4 = @(x, u) f2(x + p.Ts*k3(x,u), u);
fd2 = @(x, u) x + p.Ts/6*(k1(x,u) + 2*k2(x,u) + 2*k3(x,u) + k4(x,u));
gd2 = @(x, u) g2(x, u);

fun1 = @(xu) [fd1(xu(1:nx), xu(nx+1:nx+nu)); gd1(xu(1:nx), xu(nx+1:nx+nu))] - [xu(1:nx); r1];
fun2 = @(xu) [fd2(xu(1:nx), xu(nx+1:nx+nu)); gd2(xu(1:nx), xu(nx+1:nx+nu))] - [xu(1:nx); r2];

xu_0 = zeros(nx+nu,1);
f_sol1 = fsolve(fun1, xu_0);
f_sol2 = fsolve(fun2, xu_0);
disp("FM1 equilibrium: ")
disp(f_sol1)
disp("FM2 equilibrium: ")
disp(f_sol2)

syms iL1 iL2 vac

x = [iL1;
     iL2;
     vac];
assume(x, 'real')

syms d1 d2

u = [d1;
     d2];
assume(u, 'real')
assume([eye(nu); -eye(nu)] * u <= [1; 1; 0; 0])

xnext1 = fd1(x, u);
y1 = gd1(x, u);
xnext2 = fd2(x, u);
y2 = gd2(x, u);

sol1 = solve([xnext1; y1] == [x; r1], [x; u], 'ReturnConditions', true);
sol2 = solve([xnext2; y2] == [x; r2], [x; u], 'ReturnConditions', true);
x_e1 = double([sol1.iL1;
               sol1.iL2;
               sol1.vac]);
x_e2 = double([sol2.iL1;
               sol2.iL2;
               sol2.vac]);
u_e1 = double([sol1.d1;
               sol1.d2]);
u_e2 = double([sol2.d1;
               sol2.d2]);
y_e1 = double(subs(subs(y1, x, x_e1), u, u_e1));
y_e2 = double(subs(subs(y2, x, x_e2), u, u_e2));

A1 = jacobian(xnext1, x);
matlabFunction(A1, 'Vars', {x, u}, 'File', 'A1');
A2 = jacobian(xnext2, x);
matlabFunction(A2, 'Vars', {x, u}, 'File', 'A2');

B1 = jacobian(xnext1, u);
B2 = jacobian(xnext2, u);

C1 = jacobian(y1, x);
matlabFunction(C1, 'Vars', {x, u}, 'File', 'C1');
C2 = jacobian(y2, x);
matlabFunction(C2, 'Vars', {x, u}, 'File', 'C2');

D1 = jacobian(y1, u);
D2 = jacobian(y2, u);

% MPC controller
mpc = MPC_controller(N_MPC);

% MHE estimator
mhe = MHE_estimator(N_MHE);
